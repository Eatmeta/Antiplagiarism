...
namespace Antiplagiarism
{
    ...
    {
        private LevenshteinCalculator _levenshteinCalculator;

        [SetUp]
        public void SetUp()
        {
            _levenshteinCalculator = new LevenshteinCalculator();
        }

        private const string Ax10 = "a a a a a a a a a a";
        private const string Bx10 = "b b b b b b b b b b";
        ...
        public void Use_GetTokenDistance_ToCompareSingleTokens(string document1Token, string document2Token, double expectedDistance)
        {
            ...
            var actual = _levenshteinCalculator.CompareDocumentsPairwise(
        ...
        public void ReturnAllPairwiseDistances(IList<string> documents, params double[] expectedPairwiseDistances)
        {
            var tokenizedDocuments = documents.Select(document => document.Split(' ')
                .ToList()).ToList();
            var actual = _levenshteinCalculator.CompareDocumentsPairwise(tokenizedDocuments)
                .Select(r => r.Distance);
